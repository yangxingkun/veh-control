  ...
  const supportClick = (e, index, item) => {
    ...
    if (id == 1) {
      ...
      setMessages((pre) => {
        pre[index] = {
          ...item,
          ...{ ishand: false, isverhand: !item.isverhand },
        };
        return [...pre];
      });
    }
    Taro.nextTick(() => {
      let status = messages[index].ishand
        ? 1
        : messages[index].isverhand
        ? 2
        : 0;
      console.log(status);
      ...
      <View key={index} style={{ margin: '40px 0' }}>
        {item.role == 'user' && (
          <View className="user">
             ...
             */}
            <View
              className="taro_html k_html content"
        ...
        {item.role == 'assistant' && (
          <View className="assistant">
            <img
              className="avatar"
              src="http://152.136.205.136:9000/vehicle-control/font/Avatar%20ChatGPT.svg"
            ></img>
            <View className="content">
              ...
              <HighlightText
                ...
                ignoreCase={false}
                highlightStyle={{ padding: '3px', color: '#3388FF' }}
              >
                <Text userSelect={true} decode>{item.content}</Text>
              </HighlightText>
              <View
              ...
              >
                <img
                  data-id="1"
                  className={classNames('footer-img')}
                  src={item.ishand ? handaIconSvg : handIconSvg}
                  style={{ marginRight: '24px' }}
                />
                <img
                  data-id="2"
                  className={classNames('footer-img')}
                  src={item.isverhand ? verhandaIconSvg : verhandIconSvg}
                />
              </View>
            </View>
          </View>
